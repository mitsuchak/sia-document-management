...
$(document).ready(() => {
    (function ($) {
        ...
        function userFormValidaion() {
            // Example starter JavaScript for disabling form submissions if there are invalid fields
            // Fetch all the forms we want to apply custom Bootstrap validation styles to
            // Loop over them and prevent submission
            document.querySelectorAll('.need-validation').forEach(form => {
                form.addEventListener('submit', event => {
                    if (!form.checkValidity()) {
                        event.preventDefault()
                        event.stopPropagation()
                    ...
                    const specificDomain = '@slgroup.in'; // Replace with your specific domain
                    if (!emailValue.endsWith(specificDomain)) {
                        emailField.setCustomValidity('Please enter a valid email with the @slgroup.in domain.');
                        event.preventDefault();
                        event.stopPropagation();
                    } else {
        ...
        function fileValidaion(){
            document.querySelectorAll('.file-validaion').forEach(form => {
                form.addEventListener('submit', event => {
                    if (!form.checkValidity()) {
                        event.preventDefault()
                        event.stopPropagation()
                    ...
                    if (fileInput) {
                        ...
                        const allowedExtensions = ['.csv']; // Example allowed extensions
                        const fileExtension = fileName.substring(fileName.lastIndexOf('.')).toLowerCase();
                        if (!allowedExtensions.includes(fileExtension)) {
                            alert('Please select a valid file with extensions: ' + allowedExtensions.join(', '));
                            event.preventDefault();
                            event.stopPropagation();
                        }
                    }
